...
def fetch_batch(_epoch, batch_index, _batch_size, _scaled_housing_data_plus_bias, _label):
    print(batch_index)
    end_ind = (batch_index + 1) * _batch_size if ((batch_index + 1) * _batch_size < len(_scaled_housing_data_plus_bias)) else len(_scaled_housing_data_plus_bias)
    print("end_ind:", end_ind)
    _X_batch, _y_batch = _scaled_housing_data_plus_bias[batch_index*_batch_size:end_ind], _label[batch_index * _batch_size : end_ind]
